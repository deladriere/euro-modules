...
const uint8_t frameList[] PROGMEM = {
   ...
   _AY,60,0,20,_IY,10,0,21,_N,8,2,22, _S,8,3,21, // Eins 
 _SP,8,0,20, 
  _S,8,3,21,_WH,6,0,21,_AY,60,10,21,_IY,5,0,21, // Zwei 
 _SP,8,0,20, 
  _Tc,12,0,20,_R,10,5,20,_AY,60,10,21,_IY,6,0,20, // Drei 
 _SP,5,0,20, 
  _F,20,0,19,_IY,60,0,19,_RX,1,0,19, // Vier 
 _SP,50,0,20, 
  ...
  while(1) {
   ...
   pitchPhaseInc = pitchTable[ map( analogRead(2), 0,1024,0,64 ) ]; 
    ...
    for (;staticFrames--;) 
    {
    	...
    	delay( map( analogRead(3), 0,1024,60,2 ) );
    }
    if (tweenFrames) {
      const uint8_t* formantPos;
