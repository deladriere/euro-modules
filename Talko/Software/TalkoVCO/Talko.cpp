...
void Talkie::say(const uint8_t *addr)
	...
	do
		...
		val = map(val, 0, 1023, 1000, 10000); //was 10000
		...
		if(digitalRead(LOOP)==0) // check for Speech/Loop mode
			{
			if(digitalRead(TRIGGER)==0)
				{
				energy=0xf; // if trigger is released in Loop mode then stop the speech
				}
			}
		...
		if(energy == 0)
			...
			if(!repeat)
				...
				if(synthPeriod)
					...
					if(digitalRead(BEND)==0 & digitalRead(LOOP)==1)  /// sorry guys bending the neck here ! // adding a new condition : only if digitalRead(LOOP)==1
						{
						synthK5= map(analogRead(BENDING),20,1000,0,150);
						synthK10= map(analogRead(BENDING),20,1000,150,0);
						}
					}
				}
			}
