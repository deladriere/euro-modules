...
//a
const uint8_t spW000000 [] PROGMEM ={0xA7,0x4D,0x69,0xCA,0xB5,0xE6,0x9C,0xBE,0x8A,0xEE,0x35,0xAE,0x73,0xFA,0x26,0xBA,0xC7,0x78,0xC9,0xE9,0xAA,0xEA,0x1E,0x91,0xBA,0x67,0xA8,0xBA,0xA6,0xC5,0xEA,0x9C,0x21,0x87,0x6E,0xE7,0x8A,0x72,0xA6,0xEC,0xB6,0x82,0xB2,0x4E,0x9B,0x8A,0x9D,0x11,0x2A,0x51,0xF0};
//e
const uint8_t spW089339 [] PROGMEM ={0xA7,0x69,0x52,0x23,0xCA,0x7B,0x9D,0xA6,0x49,0xC9,0x5A,0xCB,0x7C,0xEA,0xA6,0xC5,0x23,0xB4,0xF5,0x69,0x9A,0x32,0x13,0x93,0x2E,0xA7,0xAB,0x2A,0x8C,0xD4,0x5A,0x03,0x00,0x00,0x3C};
//i
const uint8_t spW066166 [] PROGMEM ={0x27,0x3F,0x86,0x59,0x24,0x5F,0x9D,0x62,0x20,0xD2,0xAC,0xB9,0x74,0x8A,0x81,0x51,0xAB,0xFB,0xD6,0xC9,0x27,0x21,0xF5,0x9A,0xDB,0x27,0x9F,0x94,0xC5,0x72,0xAE,0x9C,0x7C,0x09,0x66,0xF6,0xB9,0x73,0x8A,0x49,0xD1,0xAA,0x77,0xE4,0xAA,0x27,0x45,0x8B,0xDA,0x42,0x69,0xCC,0x40,0x6A,0xD3,0x2D,0xC3,0x03};
//o
const uint8_t spW010780 [] PROGMEM ={0x23,0x97,0xE1,0xCA,0x51,0x96,0xAE,0x5C,0xA6,0x1E,0x05,0x6F,0x7A,0x0A,0x19,0x7A,0x12,0x35,0xE9,0x2A,0xA4,0x9B,0x69,0xA2,0xA4,0xAB,0xD4,0x6A,0xFA,0x14,0x93,0xAE,0xCA,0x8A,0xEE,0x0B,0x70,0x32,0x6A,0xC7,0xAB,0x2F,0xC8,0x09,0x3C};
//u
const uint8_t spW052811 [] PROGMEM ={0x6B,0x2D,0x8A,0xD9,0xA7,0x36,0x9E,0xB1,0x1B,0x21,0xCD,0xEE,0x7B,0xA6,0x6E,0x99,0xB8,0xBA,0xDF,0x99,0xBA,0x63,0xE6,0xAA,0x3E,0x67,0x6A,0x96,0xC4,0x3A,0x1B,0x02,0xC0,0x03};
...
/*
   ...
   12: sentences
   13: French Vowels Female
   14: French digits
   15 :French Vowels Male
   16: Voiced allophones
   17: VCO friendly voiced allophones
 */
...
void rot()
{
                ...
                interruptCount =constrain(interruptCount,0,34 ); // 2 dents per unit

                voice.bank = interruptCount/2; // 2 dents per unit
...
void digits()
{
                byte n = map(analogRead(POT_SOUND),0,1000,0,9);
...
void digitsF()
{
                byte n = map(analogRead(POT_SOUND),0,1023,0,9);
                sayNumberF(n);
}
...
//mode = digitalRead(PIN_SPEECH)<<1  + digitalRead(PIN_VCO);
                ...
                case 4:
                                vocabulary(map(analogRead(POT_SOUND),0,1023,1,53));
                                break;
                ...
                case 15:
                                vowels_FR_M(map(analogRead(POT_SOUND),0,1010,1,5));

                                break;

                case 16:
                             
                                voice.say(alphons[ map(analogRead(POT_SOUND),0,1013,0,71)]);
                               
                                break;

                case 17:
                                voice.say(alphons[vco_OK[ map(analogRead(POT_SOUND),0,1013,0,24)]]);
