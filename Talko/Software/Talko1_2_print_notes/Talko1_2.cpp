...
void Talkie::say(const uint8_t *addr)
	...
	uint8_t energy;
                ...
                note=map(analogRead(POT_SPEED),0,1013,0,126);
                                                              
                             // Serial.print("          ");
                             // Serial.print(note);
                             // Serial.print("      ");
                             // Serial.print(noteL[note]);
	...
	do
		...
		// PITCH HACK
	  long val ;
		//val = map(val, 0, 1023, 7500, 8500); //was 1000 to 10000
    val=clock;
    position(2,48);
    Serial.println(val);
...
/// vco
		...
		//delay(speed); not used in search of notes
